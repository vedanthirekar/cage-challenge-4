"""
IMPALA agent for CAGE Challenge 4
"""
...
class IMPALAAgent(BaseAgent):
    """
    IMPALA-trained agent for CAGE Challenge 4
    """
    ...
    def __init__(self, checkpoint_path: str = None, name: str = None):
        ...
        if checkpoint_path and os.path.exists(checkpoint_path):
            if IMPALAAgent._shared_algo is None or IMPALAAgent._shared_checkpoint_path != checkpoint_path:
                self._load_algorithm(checkpoint_path)
                IMPALAAgent._shared_algo = self.algo
                IMPALAAgent._shared_checkpoint_path = checkpoint_path
            else:
                self.algo = IMPALAAgent._shared_algo
        else:
            print(f"⚠️  Checkpoint not found: {checkpoint_path}")
            self.algo = None

    def _load_algorithm(self, checkpoint_path: str):
        """Load trained IMPALA algorithm"""
        try:
            import ray
            ...
            print(f"🔍 Loading IMPALA model from: {abs_path}")
            ...
            # Load checkpoint
            self.algo = IMPALA.from_checkpoint(abs_path)
            print("✅ Successfully loaded IMPALA model")

        except Exception as e:
            print(f"⚠️  Could not load IMPALA model: {e}")
            self.algo = None
        ...
        if self.algo is not None:
            try:
                ...
                if self.step_count <= 5:
                    print(f"✅ {self.name} using TRAINED IMPALA MODEL (policy: {self.policy_id})")
